  ...
  return (
    <div className="max-w-5xl mx-auto space-y-16">
      <header className="text-center space-y-4">
        <h2 className="text-5xl font-black uppercase italic tracking-tighter text-slate-900">Support & Counseling</h2>
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-slate-400 mt-1 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {supportTypes.map((type) => (
          <motion.button
            ...
            className="p-10 bg-white rounded-[3rem] border border-slate-100 shadow-sm text-left group hover:border-slate-900 hover:shadow-2xl transition-all duration-500"
          >
            <div className="w-16 h-16 rounded-2xl bg-slate-50 text-slate-900 flex items-center justify-center mb-8 group-hover:bg-slate-900 group-hover:text-emerald-400 transition-all duration-500 shadow-sm">
              <type.icon className="w-8 h-8" />
            </div>
            <h3 className="text-2xl font-black uppercase italic tracking-tighter text-slate-900 mb-3 leading-tight">{type.title}</h3>
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider leading-relaxed mb-8">{type.desc}</p>
            <div className="flex items-center gap-3 text-xs font-black uppercase italic tracking-tighter text-slate-900 group-hover:translate-x-2 transition-transform">
      ...
      <section className="bg-slate-900 rounded-[4rem] p-16 text-white relative overflow-hidden group cursor-pointer shadow-2xl shadow-slate-900/20">
        <div className="relative z-10 max-w-2xl">
          <h3 className="text-4xl font-black uppercase italic tracking-tighter mb-6 leading-tight">Emergency Support?</h3>
          ...
          <button 
            onClick={() => setSubmitted(true)}
            className="px-10 py-5 bg-emerald-400 text-black font-black uppercase italic tracking-tighter text-sm rounded-2xl hover:bg-emerald-300 transition-all shadow-xl shadow-emerald-400/20"
          >
            Contact Emergency Team
          </button>
        </div>
        <HelpCircle className="absolute -right-16 -bottom-16 w-80 h-80 text-white/5 rotate-12 group-hover:scale-110 transition-transform duration-1000" />
      </section>
    </div>
  );
}
